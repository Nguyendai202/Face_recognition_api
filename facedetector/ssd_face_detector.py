...
def model_ssd_restore_from_pb(pb_path, node_dict,GPU_ratio=None):# tải thông tin từ tệp pb lên , tệp này chứa biến và các hoạt động mô hình 
    with tf.Graph().as_default():# tạo đồ thị mặc định cho tensorflow 
        ...
        if GPU_ratio is None:
            config.gpu_options.allow_growth = True  # cho dùng hết cái có sẵn 
        else:
            config.gpu_options.per_process_gpu_memory_fraction = GPU_ratio  # giới hạn dùng gpu theo tỉ lẹ ratio 
        sess = tf.Session(config=config)
        with gfile.FastGFile(pb_path, 'rb') as f:# mở tệp pb 
...
class FaceMaskDetection():
    def __init__(self,pb_path,margin=44,GPU_ratio=0.1):
        ...
        sess, tf_dict = model_ssd_restore_from_pb(pb_path, node_dict,GPU_ratio = GPU_ratio)
